...
正式模式需要先注册或登录本机账号。账号、密码哈希、会话令牌哈希、带用户归属的聊天记录和操作日志保存在 `PROFILE_DB_PATH` 指定的 SQLite 文件中；原始密码和百炼密钥不会写入日志。每个账号的能力卡、版本快照、任务会话、评价证据和模型响应缓存另存于独立数据库。演示页面使用前端固定数据，不获得真实用户数据的免登录访问权。
...
`POST /api/v1/auth/login`、`POST /api/v1/auth/register` 允许匿名访问。健康检查、固定任务目录/定义的只读接口和接口文档也可匿名访问。其他 `/api/v1` 接口必须携带有效 Bearer 会话令牌；缺失或伪造 `X-App-Mode`（包括 `demo`）不能绕过鉴权。私有接口收到有效令牌后按真实操作记录日志，不依赖客户端是否声明正式模式。CORS 预检无需登录。

### 账号数据隔离与升级

- 默认 `PROFILE_DB_PATH=profile.db`：账号和审计数据库为 `profile.db`；私有业务数据库位于 `profile.db.users/<用户ID的SHA256>.sqlite3`。
- 私有路径仅从服务端验证后的会话用户 ID 推导，不接受请求体、查询参数或客户端用户头指定归属。
- 不同账号可使用相同能力卡 ID，各自的卡片、画像版本、任务进度和评价互不影响。访问别人的任务 ID 或尚未在自己账号保存的卡片返回 `404`；推荐接口拒绝使用其他账号的卡片。
- 模型响应缓存随业务数据库隔离：同一账号重复输入仍复用，跨账号不复用个人评价/聊天结果。
- **升级不会删除或自动认领旧数据。** 旧 `profile.db` 中没有用户归属的卡片、快照、任务与证据原样保留，但不通过账号 API 暴露。已有账号、密码及有效会话保持可用；账号的业务档案初始为空。需要迁入旧数据时，必须先核对归属并备份，不应自动绑定给第一个登录账号。
- 备份/迁移时同时备份 `PROFILE_DB_PATH` 文件和旁边的 `<文件名>.users/` 目录；在线 SQLite 应使用备份 API 或先停服务。公共 `knowledge.db` 可继续共享。
- 私有 API 和认证响应带 `Cache-Control: no-store`。静态服务只暴露前端 `dist/`，不要把后端目录、数据库、日志或 `.env` 放入静态根目录。

隔离验收（不产生模型费用，使用临时数据库，自动启动并停止本机测试服务）：

```bash
python -m pytest -q
python scripts/check_user_isolation.py
```

验收包括匿名/伪造模式拒绝访问、跨账号读写、旧库保留、缓存/日志隔离、完整任务提交的证据归属，以及 5 个账号同时使用相同卡片 ID。该修复不是完整公网安全审计；上线仍需配置 HTTPS、限流、备份和服务器权限。
...
职业探索页只读取已确认能力卡。后端先将请求拆成“目标岗位锚点 + 每张能力卡”的多个短查询，再使用百炼 `qwen3.7-text-embedding` 批量生成查询向量；向量和资料均保存在本地 SQLite，检索结果在本地做加权 RRF 融合与 MMR 去重，最后把带引用 ID 的片段交给 Qwen 生成结构化推演。查询向量按资料指纹、模型、维度和查询文本逐条缓存，同一组卡片再次探索不会重复调用 Embedding。后端还会检查每张能力卡是否被最终引用片段覆盖；没有岗位资料支持的卡片会进入未知项，不能被模型写成支持性结论。当前 `RAG_RETRIEVER_MODE=vector` 是默认策略；`adaptive` 是成本受控的实验模式，仅在任一意图的向量间隔较低或覆盖不足时调用一次 `qwen3-rerank`；`hybrid` 用于复现 SQLite FTS5 + 向量融合 + 重排的固定对照链路。前端不会直接请求百炼，也不会接触 API Key。
...
知识库资料来自项目提供的 `公共RAG知识库` 解压内容和本轮核验的官方公开页面，已按文档登记 `document_id`、知识域、资料级别、来源 URL、版本、检索日期和可信度。当前扩充批次覆盖岗位、产品案例、跨企业工作流、评测与治理方法：`jobs/international_ai_product_roles_v2.md` 汇总 Google、OpenAI、Anthropic、Databricks、Meta、Adobe，`jobs/china_ai_product_roles.md` 汇总阿里、字节、华为、京东、快手、智谱、MiniMax、DeepSeek，`cases/` 收录 Apple 与跨企业官方产品工作流，`methods/` 收录评测、治理和公共 RAG 发布规则。聚合页、历史页或正文不可见的招聘入口只保留为方向索引，不作为具体任职要求；所有摘要均保留原始链接，不复制受限原文。
...
RAG v2 的检索链路回归不需要百炼调用，覆盖查询拆分、批量缓存、RRF/MMR 融合和能力卡覆盖诊断：
